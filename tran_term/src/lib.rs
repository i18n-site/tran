...
impl Term {
  ...
  pub fn restore(&self, txt: impl AsRef<str>) -> String {
    let txt = txt.as_ref();
    code_replace(txt, |li, org, id| {
      word_push(
        li,
        if let Some(val) = self.val.get_index(id) {
          val
        } else {
          org
        },
  ...
  pub fn replace(&mut self, txt: impl AsRef<str>) -> Option<String> {
    ...
    for mat in self.ac.leftmost_find_iter(&txt_lower) {
      ...
      if let Some(val) = self.map.get(matched) {
        ...
        let (id, _) = self.val.insert_full(val);
        li.push(format!(r#"<code v="{id}">{matched}</code>"#));
      }
      pos = end;
    }
    if !li.is_empty() {
